...
One install path, one config shape, one test surface. Dual drivers and Compose were no longer justified after dockerless became the primary path. FTS5 covers lexical search; vector/semantic search remains out of scope.

## Alternatives considered

| Option | Why not |
|--------|---------|
| Keep Docker optional | Dual-mode cost outweighed benefit |
| Postgres-only | Too heavy for local single-user installs |
| Files-only (no DB) | Poor fit for search, listing, and incremental crawl |
