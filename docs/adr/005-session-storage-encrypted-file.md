...
We need to store Confluence session cookies securely so the cron job can use them for automated scraping. The storage must work portably under the local data directory (or a custom `--data-dir`).

## Decision

Use AES-GCM encrypted file storage for session cookies, with a user-provided encryption key.
