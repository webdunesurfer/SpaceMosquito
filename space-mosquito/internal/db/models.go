...
func (d *DB) CountPagesBySpaceID(ctx context.Context, spaceID uuid.UUID) (int, error) {
	var count int
	err := d.pool.QueryRow(ctx, "SELECT COUNT(*) FROM pages WHERE space_id = $1", spaceID).Scan(&count)
	if err != nil {
		if d.log.Enabled() {
			d.log.Errorw("count pages failed", "space_id", spaceID, "error", err)
		}
		return 0, err
	}
	return count, nil
...
func (d *DB) DeleteSpace(ctx context.Context, spaceKey string) error {
	space, err := d.GetSpaceByKey(ctx, spaceKey)
	if err != nil {
		return err
	}
	if _, err := d.pool.Exec(ctx, "DELETE FROM pages WHERE space_id = $1", space.ID); err != nil {
		if d.log.Enabled() {
			d.log.Errorw("delete pages failed", "space_key", spaceKey, "error", err)
		}
		return err
	}
	if _, err := d.pool.Exec(ctx, "DELETE FROM spaces WHERE id = $1", space.ID); err != nil {
		if d.log.Enabled() {
			d.log.Errorw("delete space failed", "space_key", spaceKey, "error", err)
		}
		return err
	}
	return nil
